...
namespace detail {
	...
	enum class command_type { NOP, MILESTONE, EXECUTION, PUSH, AWAIT_PUSH, REDUCTION, SHUTDOWN, SYNC };
	constexpr const char* command_string[] = {"NOP", "MILESTONE", "EXECUTION", "PUSH", "AWAIT_PUSH", "REDUCTION", "SHUTDOWN", "SYNC"};
	...
	class milestone_command final : public task_command {
		friend class command_graph;
		milestone_command(command_id cid, node_id nid, task_id tid, milestone_type type) : task_command(cid, nid, tid), type(type) {}

	  public:
		milestone_type get_milestone_type() const { return type; }
	...
	using command_data = std::variant<nop_data, milestone_data, execution_data, push_data, await_push_data, reduction_data, shutdown_data, sync_data>;
