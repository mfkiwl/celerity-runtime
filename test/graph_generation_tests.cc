...
namespace detail {
	...
	TEST_CASE("graph_generator allows chunks to require empty buffer ranges", "[graph_generator][command-graph]") {
		...
		CHECK(inspector.get_commands(tid_b, std::nullopt, command_type::EXECUTION).size() == 3);
		const auto computes_node1 = inspector.get_commands(tid_b, node_id(1), command_type::EXECUTION);
		CHECK(computes_node1.size() == 1);
		const auto computes_node2 = inspector.get_commands(tid_b, node_id(2), command_type::EXECUTION);
